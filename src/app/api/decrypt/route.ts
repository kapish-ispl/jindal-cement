...
export async function POST(req: Request) {
  const { responseFromApi } = await req.json();
...
-----END PRIVATE KEY-----`;
  if (!rawPrivateKey) {
    return NextResponse.json({ error: 'Missing PRIVATE_KEY in environment' }, { status: 500 });
  }
  const privateKey = rawPrivateKey.replace(/\\n/g, '\n');

  const aesKey = crypto.privateDecrypt(
    { key: privateKey, padding: crypto.constants.RSA_PKCS1_OAEP_PADDING }, //RSA_PKCS1_OAEP_PADDING RSA_PKCS1_PADDING
    encryptedKey
  );
